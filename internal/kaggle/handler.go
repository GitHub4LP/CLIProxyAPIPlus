...
func (h *Handler) RegisterRoutes(router gin.IRouter) {
	...
	log.Info("Kaggle proxy routes registered")
}
...
func (h *Handler) ClaudeMessages(c *gin.Context) {
	...
	if model != kaggleModel {
		log.Infof("Claude model mapping: %s → %s", model, kaggleModel)
	}
...
func (h *Handler) GeminiHandler(c *gin.Context) {
	...
	if modelName != kaggleModel {
		log.Infof("Gemini request: %s → %s:%s", modelName, kaggleModel, method)
	}
